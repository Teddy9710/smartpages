...
param(
    [string]$DraftPath = "",
    [string]$CoverImageUrl = "https://raw.githubusercontent.com/Teddy9710/smartpages/main/docs/assets/readme-hero.png",
    [switch]$Yes
)

if ([string]::IsNullOrWhiteSpace($DraftPath)) {
    $DraftPath = Join-Path $PSScriptRoot "international-2026-08-07\devto.md"
}
...
if (-not (Test-Path -LiteralPath $reviewReceiptPath -PathType Leaf)) {
    throw "HumanWriting review receipt is missing. Ask Codex to optimize this draft with the HumanWriting skill before sending: $reviewReceiptPath"
}

$expectedReviewHash = (Get-Content -Raw -LiteralPath $reviewReceiptPath).Trim().ToLowerInvariant()
$actualDraftHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $resolvedDraftPath).Hash.ToLowerInvariant()
if ($expectedReviewHash -notmatch '^[a-f0-9]{64}$' -or $expectedReviewHash -ne $actualDraftHash) {
    throw "The draft changed after its HumanWriting review. Ask Codex to review it again before sending."
}
...
if (-not $frontMatterMatch.Success) {
    throw "Draft must contain YAML front matter followed by Markdown content: $resolvedDraftPath"
}
...
if ([string]::IsNullOrWhiteSpace($title)) {
    throw "The draft front matter is missing a title."
}

if ([string]::IsNullOrWhiteSpace($bodyMarkdown)) {
    throw "The draft body is empty."
}
...
if ($tags.Count -gt 4) {
    throw "DEV accepts at most four tags; the draft contains $($tags.Count)."
}

Write-Host "DEV draft preview" -ForegroundColor Cyan
Write-Host "  Title: $title"
Write-Host "  Tags: $($tags -join ', ')"
Write-Host "  Source: $resolvedDraftPath"
...
$headers = @{
    ...
    "User-Agent" = "SmartPages-promotion-script/1.0"
}
